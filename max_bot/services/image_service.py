...
class ImageGenerationService:
    ...
    def _upload_image(self, image_data: str, client) -> str:
        ...
        response = client.post(
            f"{self._base_url}/upload-init", headers=headers, files=files
        )
    ...
    def generate(self, prompt: str, width: int = 512, height: int = 512) -> dict:
        ...
        try:
            with httpx.Client(timeout=120) as client:
                ...
                response = client.post(
                    f"{self._base_url}/generations", headers=headers, json=payload
                )
                ...
                for _ in range(60):
                    ...
                    status = status_data.get("generationJob", {}).get(
                        "status", "UNKNOWN"
                    )

                    if status == "COMPLETE":
                        images = status_data.get("generationJob", {}).get(
                            "generated_images", []
                        )
                        if images:
        ...
        try:
            with httpx.Client(timeout=180) as client:
                ...
                if init_resp.status_code != 200:
                    logger.error(
                        f"Init error: {init_resp.status_code} - {init_resp.text}"
                    )
                ...
                if not image_id:
                    return {
                        "error": "No image ID returned",
                        "data": str(init_data)[:200],
                    }
                ...
                files = {
                    "file": ("image.jpg", base64.b64decode(image_data), "image/jpeg")
                }

                upload_resp = client.post(upload_url, data=fields, files=files)

                if upload_resp.status_code not in (200, 204):
                    logger.error(
                        f"S3 upload error: {upload_resp.status_code} - {upload_resp.text}"
                    )
                ...
                response = client.post(
                    f"{self._base_url}/generations", headers=gen_headers, json=payload
                )

                if response.status_code != 200:
                    logger.error(
                        f"Leonardo edit error: {response.status_code} - {response.text}"
                    )
                    return {
