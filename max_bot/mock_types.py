...
class MockPhotoSize:
    def __init__(
        self, file_id: str, width: int = 0, height: int = 0, file_size: int = 0
    ):
...
class MockMessage:
    def __init__(self, message_dict: dict):
        ...
        self.chat = (
            MockChat(message_dict.get("chat")) if message_dict.get("chat") else None
        )
        self.from_user = (
            MockUser(message_dict.get("from")) if message_dict.get("from") else None
        )
        ...
        self.photo = (
            [MockPhotoSize(**p) for p in message_dict.get("photo", [])]
            if message_dict.get("photo")
            else None
        )
...
class MockUpdate:
    def __init__(self, update_dict: dict):
        ...
        self.message = (
            MockMessage(update_dict.get("message"))
